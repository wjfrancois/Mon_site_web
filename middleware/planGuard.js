...
function guardBarberLimit(req, res, next) {
  if (req.method !== 'POST') return next();
  const plan = getPlan(req.tenant?.plan);
  if (plan.maxBarbers === -1) return next();
  const count = db.prepare('SELECT COUNT(*) as c FROM barbers WHERE tenant_id = ? AND active = 1').get(req.tenantId);
...
function guardSmsQuota(tenantId) {
  const tenant = db.prepare('SELECT * FROM tenants WHERE id = ?').get(tenantId);
  const plan = getPlan(tenant?.plan);
  const now = new Date().toISOString().slice(0,7);
  if (tenant.sms_reset_date !== now) {
    db.prepare('UPDATE tenants SET sms_used_this_month = 0, sms_reset_date = ? WHERE id = ?').run(now, tenantId);
    return true;
  }
  return isWithinLimit(tenant.sms_used_this_month, plan.smsMonth);
}

function incrementSmsUsage(tenantId) {
  db.prepare('UPDATE tenants SET sms_used_this_month = sms_used_this_month + 1 WHERE id = ?').run(tenantId);
}
