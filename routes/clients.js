...
router.post('/', (req, res) => {
  ...
  const result = db.prepare('INSERT INTO clients (name, phone, email, notes) VALUES (?, ?, ?, ?)').run(name, phone, email || null, notes || null);
  res.json({ id: result.lastInsertRowid, message: 'Client ajouté' });
});

router.put('/:id', (req, res) => {
  const { name, phone, email, notes } = req.body;
  db.prepare('UPDATE clients SET name=?, phone=?, email=?, notes=? WHERE id=?').run(name, phone, email || null, notes || null, req.params.id);
