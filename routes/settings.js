...
router.put('/:key', (req, res) => {
  ...
  db.prepare('INSERT INTO site_settings (key, value, updated_at) VALUES (?, ?, datetime(\'now\')) ON CONFLICT(key) DO UPDATE SET value = excluded.value, updated_at = excluded.updated_at').run(req.params.key, value);
...
router.post('/upload/hero', upload.single('image'), (req, res) => {
  ...
  const old = db.prepare('SELECT value FROM site_settings WHERE key = ?').get('hero_image');
  ...
  db.prepare('INSERT INTO site_settings (key, value) VALUES (?, ?) ON CONFLICT(key) DO UPDATE SET value = excluded.value').run('hero_image', imageUrl);
  res.json({ url: imageUrl, message: 'Image mise à jour' });
});

// DELETE hero image (reset to default)
router.delete('/upload/hero', (req, res) => {
  const old = db.prepare('SELECT value FROM site_settings WHERE key = ?').get('hero_image');
  if (old?.value && old.value.startsWith('/img/uploads/')) {
    const oldPath = path.join(__dirname, '..', 'public', old.value);
    if (fs.existsSync(oldPath)) fs.unlinkSync(oldPath);
  }
  db.prepare('UPDATE site_settings SET value = ? WHERE key = ?').run('', 'hero_image');
